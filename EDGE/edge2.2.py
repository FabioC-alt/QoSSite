...
USE_HIGH_FUNCS = 0   # <-- cambia questo numero (max 8)
USE_LOW_FUNCS = 1    # <-- cambia questo numero (max 5)

FAAS_HIGH = ALL_FAAS_HIGH[:USE_HIGH_FUNCS]
FAAS_LOW = ALL_FAAS_LOW[:USE_LOW_FUNCS]
...
# ---------- Queue Processing con priorità assoluta ----------
def process_all_queues():
    while True:
        request_data = None
        queue_name = None
        with queue_lock:
            if high_priority_queue:  # HIGH sempre prima
...
def log_queue_counts():
    while True:
        try:
            with queue_lock:
                total_high = len([r for r in high_priority_queue if r["priority_level"].lower() == "high"]) + \
                             len([r for r in low_priority_queue if r["priority_level"].lower() == "high"])
                total_low = len([r for r in high_priority_queue if r["priority_level"].lower() != "high"]) + \
                            len([r for r in low_priority_queue if r["priority_level"].lower() != "high"])
...
def run_server():
    ...
    # Start unico worker con priorità assoluta
    threading.Thread(target=process_all_queues, daemon=True).start()

    threading.Thread(target=log_queue_counts, daemon=True).start()

    print(f"Server running at http://{host}:{port}")
    print(f"High priority functions in use: {FAAS_HIGH}")
    print(f"Low priority functions in use: {FAAS_LOW}")
