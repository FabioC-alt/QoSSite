    ...
    with queue_lock:
        if len(high_priority_queue) < MAX_HIGH_QUEUE:
            ...
            if len(low_priority_queue) < MAX_LOW_QUEUE:
                ...
                # Case 1: low queue has some LOW -> drop them
                ...
                if dropped_request:
                    ...
                    # Case 2: both queues are full of HIGH requests
                    oldest_high = high_priority_queue.popleft()  # remove oldest from high queue
                    try:
                        ...
                        response = requests.post(executor_url, json=oldest_high, headers=headers, timeout=10)
                        print(f"Forwarded OLDEST HIGH request {oldest_high['request_number']} to BACKUP edge {BACKUP_EDGE_IP} - Status: {response.status_code}")
                        forwarded_high_to_backup += 1
                    except requests.exceptions.RequestException as e:
                        print(f"Error forwarding HIGH request {oldest_high['request_number']} to backup: {e}")
                        dropped_high_in_low += 1

                    # Insert the new high request in high queue
                    high_priority_queue.append(request_data)
    ...
    while True:
        ...
        try:
            with queue_lock:
                ...
                for req in list(low_priority_queue):  # safer copy
