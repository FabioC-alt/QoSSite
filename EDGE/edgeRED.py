...
BACKUP_EDGE_IP = "192.168.17.89"  # Backup edge node for overflow
...
class CustomHandler(SimpleHTTPRequestHandler):
            ...
            with queue_lock:
                if priority_level.lower() == "high":
                    if len(high_priority_queue) < MAX_HIGH_QUEUE:
                        ...
                        # Both queues full → forward to backup
                        self.forward_to_backup(request_data)
                else:  # Low-priority
                    if len(low_priority_queue) < MAX_LOW_QUEUE:
                        ...
                        # Low queue full → forward to backup
                        self.forward_to_backup(request_data)
    ...
    @staticmethod
    def forward_to_backup(request_data):
        """Forward request to backup edge node."""
        try:
            ...
            print(f"Forwarded {request_data['priority_level'].upper()} request {request_data['request_number']} to BACKUP edge {BACKUP_EDGE_IP} - Status: {response.status_code}")
        except requests.exceptions.RequestException as e:
            print(f"Error forwarding request {request_data['request_number']} to backup: {e}")


def forward_request(request_data):
    """Send request to main executor node."""
