...
for file, header in [(SNAPSHOT_FILE, ["timestamp","high_in_high","low_in_high","high_in_low","low_in_low","forwarded_high_to_backup","forwarded_low_to_backup"]),
                     (LOG_FILE, ["timestamp","total_queue","high_in_high","low_in_high","high_in_low","low_in_low","dropped_high_in_low","dropped_low","forwarded_high_to_backup","forwarded_low_to_backup"])]:
...
def insert_high_request(request_data):
    """Insert high-priority request respecting the new rules."""
    global dropped_high_in_low, dropped_low
    with queue_lock:
        if len(high_priority_queue) < MAX_HIGH_QUEUE:
            ...
            if len(low_priority_queue) < MAX_LOW_QUEUE:
                ...
                # Drop last low-priority request in low queue
...
def insert_low_request(request_data):
    """Insert low-priority request or forward if queue full."""
    ...
    with queue_lock:
        if len(low_priority_queue) < MAX_LOW_QUEUE:
            ...
            try:
                ...
                response = requests.post(executor_url, json=request_data, headers=headers, timeout=10)
                print(f"Forwarded LOW request {request_data['request_number']} to BACKUP edge {BACKUP_EDGE_IP} - Status: {response.status_code}")
...
def forward_request(request_data):
    ...
    headers = {"Host": host_header, "Content-Type": "application/json"}
    try:
        response = requests.post(executor_url, json=request_data, headers=headers, timeout=30)
...
def process_low_queue():
    while True:
        ...
        try:
            with queue_lock:
                # Promote high-priority requests to high queue if possible
...
def log_queue_counts():
    ...
    while True:
        try:
            ...
            print(f"[{timestamp}] Total queue: {total_queue}, High in HIGH: {high_in_high}, Low in HIGH: {low_in_high}, High in LOW: {high_in_low}, Low in LOW: {low_in_low}, Dropped HIGH in LOW: {dropped_high_in_low}, Dropped LOW: {dropped_low}, Forwarded LOW: {forwarded_low_to_backup}")
