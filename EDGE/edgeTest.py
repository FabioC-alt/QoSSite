...
CLOUD_FORWARD_DELAY = 0.2  # seconds
...
# ---------- FaaS Deployments ----------
ALL_FAAS_HIGH = [
    "highpriorityfunc", "highpriorityfunc2", "highpriorityfunc3",
    "highpriorityfunc4", "highpriorityfunc5", "highpriorityfunc6",
    "highpriorityfunc7", "highpriorityfunc8"
]

ALL_FAAS_LOW = [
    "lowpriorityfunc", "lowpriorityfunc2", "lowpriorityfunc3",
    "lowpriorityfunc4", "lowpriorityfunc5"
]
...
def send_post_with_retry(url, headers, payload, request_data, to_cloud=False):
...
def forward_request(request_data, priority_level, to_cloud=False):
    ...
    if to_cloud and CLOUD_FORWARD_DELAY > 0:
        time.sleep(CLOUD_FORWARD_DELAY)

    send_post_with_retry(executor_url, headers, payload, request_data, to_cloud=to_cloud)
    print(f"Forwarded {priority_level.upper()} request {request_data['request_number']} to {target} {'(CLOUD)' if to_cloud else '(EDGE)'}")
            ...
            with queue_lock:
                if priority_level.lower() == "high":
                    ...
                    if len(high_priority_queue) < MAX_HIGH_QUEUE:
                        ...
                        threading.Thread(target=forward_request, args=(request_data, "high", True), daemon=True).start()
                    ...
                    if len(low_priority_queue) < MAX_LOW_QUEUE:
                        ...
                        threading.Thread(target=forward_request, args=(request_data, "low", True), daemon=True).start()
                        self.send_response(200)
                        self.end_headers()
                        return
                    else:
...
# ---------- Unified Scheduler ----------
# ---------- Unified Scheduler (Modified for testing cloud offload) ----------
def unified_scheduler():
    """
    Processes both HIGH and LOW queues fairly.
    Slows down processing to allow the high-priority queue to fill
    and trigger cloud offload for testing.
    """
    while True:
        request_data = None
        with queue_lock:
        ...
        if request_data:
            ...
            threading.Thread(
                ...
                args=(request_data, request_data["priority_level"], False),
                daemon=True
            ).start()

            # ---------- Added sleep for testing ----------
            # Slow down processing so the queue can accumulate
            time.sleep(1.0)  # 1 second per request
        else:
            time.sleep(0.01)
...
def run_server():
    ...
    threading.Thread(target=unified_scheduler, daemon=True).start()
    ...
    print(f"Cloud forwarding: {'ENABLED' if ALLOW_CLOUD_FORWARD else 'DISABLED'} "
          f"(with {CLOUD_FORWARD_DELAY:.3f}s delay)")
