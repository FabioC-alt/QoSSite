...
if not os.path.exists(LATENCY_FILE):
    with open(LATENCY_FILE, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["request_number", "priority_level", "latency_seconds", "timestamp"])
...
def forward_request(request_data):
    """Forward request to CLOUD and log latency."""
    ...
    try:
        ...
        print(f"Forwarded {request_data['priority_level']} request {request_data['request_number']} "
              f"to CLOUD - Latency: {latency:.6f}s")
            ...
            if request_data:
                ...
                try:
                    ...
                    print(f"Processed {request_data['priority_level']} request {request_data['request_number']} - "
                          f"Status: {response.status_code}, Latency: {latency:.6f}s")
                except requests.exceptions.RequestException as e:
                    print(f"Error forwarding request {request_data['request_number']}: {e}")
            else:
                time.sleep(0.1)
