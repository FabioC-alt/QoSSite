...
CLOUD_FORWARD_DELAY = 0.2  # seconds of artificial delay before sending to cloud
...
def log_cloud_latency(request_data, latency):
    ...
    try:
        ...
        print(f"Error logging cloud latency: {e}")
...
def forward_request(request_data, priority_level, to_cloud=False):
    ...
    # Artificial delay before cloud forwarding
    if to_cloud and CLOUD_FORWARD_DELAY > 0:
        print(f"Delaying {CLOUD_FORWARD_DELAY:.3f}s before forwarding request {request_data['request_number']} to CLOUD")
            ...
            with queue_lock:
                if priority_level.lower() == "high":
                    ...
                    if len(high_priority_queue) < MAX_HIGH_QUEUE:
                        ...
                        if ALLOW_CLOUD_FORWARD:
                            forward_request(request_data, "high", to_cloud=True)
                            self.send_response(200)
                            self.end_headers()
                            return
                        else:
                            print(f"Rejected HIGH request {request_number} (cloud forwarding disabled)")
                            self.send_response(503)
                            self.end_headers()
                            return
                else:
                    ...
                    if len(low_priority_queue) < MAX_LOW_QUEUE:
                        ...
                        if ALLOW_CLOUD_FORWARD:
                            ...
                            print(f"Rejected LOW request {request_number} (cloud forwarding disabled)")
                            self.send_response(503)
                            self.end_headers()
                            return
